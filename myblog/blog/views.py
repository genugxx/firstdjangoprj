...
def post_new(request):
    ...
    # GET방식으로 들어온 요청인지 POST 방식으로 들어온 요청인지 검사.
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # 글 다 작성했으면 detail 페이지에서 쓴 글 확인.
            return redirect('post_detail', pk = post.pk)
    
    else:
        # 만들어놨던 폼 양식을 가져오기 위해서는
        # 변수 = 폼양식() 를 써야 한다. ()가 우측에 붙음에 ㅈ의.
        # 현재는 PostForm() 양식을 따라 만들것임을 보여준다.
        form = PostForm()
...
def post_edit(request, pk):
    ...
    # 글을 수정해야 하기 때문에 기존에 입력되어 있던 자료를 가져오는게 먼저.
    post = get_object_or_404(Post, pk=pk)
    ...
    if request.method == "POST":
        ...
        # 만약 POST방식이 아닌 경우라면 get방식 이므로 수정 직전상태.
        # 따라서 폼으로 다시 연결해줘야한다. 이때의 폼은 수정용 폼이며 
        # 수정용 폼에는 기존에 써놨었던 글이 먼저 입력되어 있어야 하므로
        # 감안해서 기존 글의 내용이 담겨있는 post를 폼에 instance로 넘겨준다.
